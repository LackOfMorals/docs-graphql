...
=== Removed support for the Object Graph Mapper (OGM)
...
The deprecated quality filters will be removed in version 7.
...
The `_EQ_` filters can be disabled from the generated schema with the option `implicitEqualFilters` set to `true`:
...
The `options` argument can be disabled from the generated schema with the option `deprecatedOptionsArgument` set to `true`:
...
The `directed` argument can be disabled from the generated schema with the option `directedArgument` set to `true`:
...
`typename_IN` can be disabled from the generated schema with the option `typename_IN` set to `true`:
...
ID fields in aggregations can be disabled from the generated schema with the option `idAggregations` set to `true`:
...
mutation {
  updateMovies(
    where: { title_EQ: "Matrix" }
    update: {
      actors: {
        connect: { overwrite: true, where: { node: { name_EQ: "Keanu" } } }
      }
    }
  ) {
    movies {
...
mutation CreateActors {
  updateActors(
    update: {
      movies: {
        connectOrCreate: [{ where: { node: { title_EQ: "The Matrix" } } }]
      }
    }
  ) {
    actors {
...
`connectOrCreate` mutations can be disabled from the generated schema with the option `connectOrCreate` set to `true`:
...
----
mutation {
  updateMovies(update: { title: "The Matrix" }) {
    movies {
      title
    }
  }
...
Implicit set input fields can be disabled from the generated schema with the option `implicitSet` set to `true`:
