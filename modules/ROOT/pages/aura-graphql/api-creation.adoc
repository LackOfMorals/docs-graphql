...
CORS is a browser-based security mechanism that prevents web pages from accessing resources from a server on a different domain.
Allow the domain that serves the browser application by adding it to the CORS policy.
...
[NOTE]
====
The domain entered in the CORS policy must be an exact match.
For example, http://localhost is not the same as http://localhost:3000/.
Wildcards are not supported. 
====
+
To add a CORS policy entry, enter the exact domain, including HTTP/S and any port number, in the **Origin box**.
...
[NOTE]
====
It is not recommended to use API keys with browser-based applications that are using the GraphQL API.
This is due to the API key being visible when using browser developer tooling and very little control over GraphQL operations.
...
. Sizing
+
Sizing a GraphQL API is based on the complexity of the type definitios, roughly their size, and the workload that is serviced.
...
There is a difference: only an API Key is used for the authentication provider.
This can be changed in the Console after the GraphQL API has been provisioned.
